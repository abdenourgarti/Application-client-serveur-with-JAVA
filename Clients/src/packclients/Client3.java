...
public class Client3 {
	...
	public static void main(String[] args) throws Exception {
		  String[] serviceChain = {"S10", "S3", "S6", "S4", "S2", "S7", "S0", "S3", "S12", "S8", "S7", "S0", "S6", "S5", "S3", "S2", "S1", "S7", "S0", "S4","FIN"};
		  String reponse;
		  try {
	            ...
	            while (index < serviceChain.length && !(service = serviceChain[index]).equals("FIN")) {
	            	...
	            	if (hasToken) {                	
	                    reponse = i.sendRequest(service, 3);
	                    ...
	                    // Wait for the token
	                    System.out.println("Client 3 waiting for the token.");
	                    waitForToken();
	                    if (hasToken) {                    	
	                        reponse = i.sendRequest(service, 3);
	                        System.out.println("service " + service + " ==> réponse : " + reponse);
	            ...
	            System.out.println("Fin client 3");
	            udpSocket.close();
    ...
    private static void waitForToken() throws IOException, InterruptedException {
        do {
        	DatagramSocket udpSocketS = new DatagramSocket(9878);
