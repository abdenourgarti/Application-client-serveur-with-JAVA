...
public class Client2 {
    ...
    public static void main(String[] args) throws Exception {
        String[] serviceChain = {"S15", "S1", "S4", "S2", "S3", "S6", "S7", "S10", "S1", "S13", "S6", "S14", "S2", "S0", "S1", "S3", "S2", "S7", "S0", "S1", "FIN"};
        String reponse;
        ...
        try {
            ...
            while (index < serviceChain.length && !(service = serviceChain[index]).equals("FIN")) {
            	if (hasToken) {                	
                    reponse = i.sendRequest(service, 2);
                    ...
                    // Wait for the token
                    System.out.println("Client 2 waiting for the token.");
                    waitForToken();
                    if (hasToken) {                    	
                        reponse = i.sendRequest(service, 2);
                        System.out.println("service " + service + " ==> réponse : " + reponse);
            ...
            System.out.println("Fin client 2");
            udpSocket.close();
    ...
    private static void waitForToken() throws IOException, InterruptedException {
        do {
        	DatagramSocket udpSocketS = new DatagramSocket(9877);
