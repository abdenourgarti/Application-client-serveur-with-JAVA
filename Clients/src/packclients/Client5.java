...
public class Client5 {
	...
	public static void main(String[] args) throws Exception {
		  String[] serviceChain = {"S2", "S14", "S1", "S7", "S6", "S3", "S0", "S5", "S2", "S15", "S7", "S0", "S4", "S12", "S3", "S6", "S7", "S2", "S1", "S13","FIN"};
		  String reponse;
		  try {
	            ...
	            while (index < serviceChain.length && !(service = serviceChain[index]).equals("FIN")) {
	            	...
	            	if (hasToken) {                	
	                    reponse = i.sendRequest(service, 5);
	                    ...
	                    // Wait for the token
	                    System.out.println("Client 5 waiting for the token.");
	                    waitForToken();
	                    if (hasToken) {                    	
	                        reponse = i.sendRequest(service, 5);
	                        System.out.println("service " + service + " ==> réponse : " + reponse);
	            ...
	            System.out.println("Fin client 5");
	            udpSocket.close();
    ...
    private static void waitForToken() throws IOException, InterruptedException {
        do {
        	DatagramSocket udpSocketS = new DatagramSocket(9880);
